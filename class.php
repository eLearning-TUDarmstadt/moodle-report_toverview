...
class overview {
	...
	public function Kurse() {
		...
		foreach ($courses as $key => $value) {
			...
			if($value->visible == 0) {
				$template = str_replace('#visibilityImageURL#', '/report/toverview/pix/hide.png', $template);
			}	
			else {
				$template = str_replace('#visibilityImageURL#', '/report/toverview/pix/show.png', $template);
			}
			$puffer .= $template;									
		}
		echo $puffer;
		...
		echo '
			<html>
				<head>
					...
					<link href="/report/toverview/css/navbar-fixed-top.css" rel="stylesheet">
					<link href="/report/toverview/css/grid.css" rel="stylesheet">
				</head>
				<body>
			';
	}
	...
	public function __destruct() {
		echo '
			...
			<script src="/report/toverview/js/jquery.min.js"></script>
			<!-- Include all compiled plugins (below), or include individual files as needed -->
   			<script src="/report/toverview/js/bootstrap.min.js"></script>	
			';
		$this->footer();
	}
...
$tOverview = new overview($class_methods);
